...
% folder, finds the ratio of their first half and second half of movement  
% distance traveled over their total distance traveled per trial, averages 
% these per block, and then creates a box plot of all 15 subjects' ratios
% per block. Individual subject means are overlayed on box plot.
        ...
        for b = 1:size(blocks,2)
            ...
            if size(ListofTrlFiles) == 0
                ...
                for trials = 1:size(ListofTrlFiles)
                    ...
                    % Calculate half distance traveled
                    half = distance/2;

                    % While dist is less than half distance, add to initial
                    i = 1;
                    dist = 0;
                    while dist < half
                        x_1 = x_pos_all(i);
                    ...
                    % Save index of halfway distance
                    index = i;

                    % While dist is greater than half distance, add to last
                    while i < (num_time_samples-1)
                        x_1 = x_pos_all(i);
                        x_2 = x_pos_all(i+1);
                        y_1 = y_pos_all(i);
                        y_2 = y_pos_all(i+1);
...
title('State: Percent of Total Distance Traveled in Second Half of Distance')
xlabel('Condition')
...
title('Joint: Percent of Total Distance Traveled in Second Half of Distance')
xlabel('Condition')
ylabel('Percent of Total Trial Distance')
xlim([0 10])  
ylim([0 1])
...
saveas(gcf, 'images/All_Subs/All_Perc_HalfDist.png')
saveas(gcf, 'images/All_Subs/All_Perc_HalfDist.svg')
