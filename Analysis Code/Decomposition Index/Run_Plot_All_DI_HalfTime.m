...
% folder, finds their average DI by time per block, and then creates an 
% error plot for all 15 subjects' DIs by time per block.
        ...
        for b = 1:size(blocks,2)
            ...
            if size(ListofTrlFiles) == 0
                ...
                for trials = 1:size(ListofTrlFiles)
                    ...
                    % Collect DI data
                    [DIj_all, DIe_all, DIj_init, DIe_init, DIj_last, DIe_last] = DIalgorithmByTime(file.TargetData, file.ExamineeData);
...
xlim([0 10])
ylim([0.25 0.65])
title(sprintf('Decomposition Index of Each Half of Movement Time (Mean %c 1 SE)', char(177)));
xlabel('Block')
...
saveas(gcf, 'images/All_Subs/All_DI_HalfTime.png')
saveas(gcf, 'images/All_Subs/All_DI_HalfTime.svg')
